...
class WordGenerator:
    ...
    def __init__(self, lm, **kwargs):
        ...
        if lm == 's':
            ...
            self.nlm = NeuralLanguageModel(params=parameters,
    ...
    def run(self, input_sequence):
        ...
        input_tokens = [word for word in input_tokens if word not in
                        self.nlm.low_freq_words]
        if set(input_tokens).issubset(set(self.nlm.dictionary)):
            encoded_input = one_hot(input_tokens, len(self.dictionary))
            word = self.nlm.model.predict(encoded_input)
            return word
        else:
